...
// Kategori-spesifik alanlar (AddProductPage ile aynı)
...
const EditProductPage = () => {
  ...
  const loadProductData = async () => {
    ...
    try {
      ...
      console.log('📦 Loading product data for edit:', id);
      ...
      if (product) {
        // Load images
        let imagesArray: Array<{ url: string; isPrimary: boolean }> = [];
        if (product.images && Array.isArray(product.images)) {
          imagesArray = product.images.map((img: any, index: number) => {
        ...
        const district = typeof location === 'string' ? '' : (location.district || '');
        ...
        if (categoryData.harvest) {
          // Convert date string to YYYY-MM-DD format
          try {
            const date = new Date(categoryData.harvest);
            if (!isNaN(date.getTime())) {
              categoryData.harvest = date.toISOString().split('T')[0];
            }
          } catch (e) {
            console.warn('Could not parse harvest date:', categoryData.harvest);
          }
        }
        if (categoryData.productionDate) {
          try {
            const date = new Date(categoryData.productionDate);
            if (!isNaN(date.getTime())) {
              categoryData.productionDate = date.toISOString().split('T')[0];
            }
          } catch (e) {
            console.warn('Could not parse production date:', categoryData.productionDate);
          }
        }
        if (categoryData.expiryDate) {
          try {
            const date = new Date(categoryData.expiryDate);
            if (!isNaN(date.getTime())) {
              categoryData.expiryDate = date.toISOString().split('T')[0];
            }
          } catch (e) {
            console.warn('Could not parse expiry date:', categoryData.expiryDate);
          }
        }
  ...
  const handleCategorySelect = (categoryId: string) => {
    ...
    setFormData(prev => ({
      ...prev,
      category: categoryId,
      unit: categoryConfig?.unit?.[0] || prev.unit,
      // Don't reset categoryData when editing - keep existing data
    }));
  };
      ...
      for (const file of imageFiles) {
        try {
          ...
          uploadedImages.push({
            ...
            isPrimary: formData.images.length === 0 && uploadedImages.length === 0 // First image is primary if no existing images
          });
        } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(`Ürün güncellenirken bir hata oluştu: ${errorMessage}`);
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="text-gray-500">Ürün yükleniyor...</div>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white border-b border-gray-200 sticky top-16 z-40">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <h1 className="text-xl font-bold text-gray-900">Ürünü Düzenle</h1>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            <h2 className="text-lg font-bold text-gray-900 mb-6">Kategori</h2>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-4">
                ...
                {formData.images.map((img, index) => (
                  <div key={index} className="relative group">
                    <img
                      ...
                      alt={`Upload ${index + 1}`}
                      className="w-32 h-32 object-cover rounded-xl border-2 border-gray-200 cursor-pointer hover:border-primary transition-all"
                      onError={(e) => {
                    ...
                    {!img.isPrimary && (
                      <button
                        type="button"
                        onClick={() => setPrimaryImage(index)}
                        className="absolute top-2 left-2 bg-gray-800/70 text-white text-xs px-2 py-1 rounded-full font-semibold hover:bg-gray-900 opacity-0 group-hover:opacity-100 transition-opacity"
                      >
                        Ana Yap
                      </button>
                    )}
              ...
              <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:bg-gray-50 transition-colors">
                ...
                <span className="text-sm text-gray-600">Yeni görsel ekle</span>
                <input
                  type="file"
                  multiple
                  accept="image/*"
