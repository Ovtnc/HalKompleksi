...
async def run_test():
    ...
    try:
        ...
        # -> Enter valid credentials for buyer role and click login.
        frame = context.pages[-1]
        # Enter email for buyer role
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Login Successful for SuperUser')).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Login was not successful for all roles using correct username/email and password as per the test plan.")
