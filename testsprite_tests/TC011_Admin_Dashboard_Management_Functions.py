...
async def run_test():
    ...
    try:
        ...
        # -> Input admin credentials and click login to open the Admin Dashboard.
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Seller Approval Successful').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: The admin dashboard did not update the seller status appropriately after approval, indicating a failure in the seller approval process as per the test plan.')
