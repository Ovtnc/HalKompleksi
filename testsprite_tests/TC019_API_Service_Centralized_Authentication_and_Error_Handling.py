...
async def run_test():
    ...
    try:
        ...
        # -> Input valid credentials and click login to obtain JWT token.
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=JWT Authentication Successful').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: API calls did not include valid JWT authentication headers or error handling was improper as per the test plan.")
