...
async def run_test():
    ...
    try:
        ...
        # -> Input buyer email and password, then click login button.
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Product request created successfully').first).to_be_visible(timeout=3000)
        except AssertionError:
            raise AssertionError("Test case failed: The product request creation or notification process did not complete successfully as per the test plan. Buyers could not create product requests or sellers did not receive/view notifications.")
