...
async def run_test():
    ...
    try:
        ...
        # -> Input email and password, then click login to log in as buyer.
        ...
        frame = context.pages[-1]
        # Click login button to log in as buyer
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Order Notification Received').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: Buyers and sellers did not receive real-time notifications for orders, product requests, and system updates without delay as required by the test plan.')
