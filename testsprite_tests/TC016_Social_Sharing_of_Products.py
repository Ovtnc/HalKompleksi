...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Product shared successfully on social media!').first).to_be_visible(timeout=30000)
        except AssertionError:
            raise AssertionError('Test case failed: The test plan execution failed to verify that the shared content contains the correct product image, title, and description on external social media platforms.')
